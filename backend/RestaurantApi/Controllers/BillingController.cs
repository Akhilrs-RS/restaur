    ...
    public async Task<IActionResult> GetReceiptData(int orderId)
    {
        ...
        var receipt = new
        {
            ...
            Invoice = new
            {
                ...
                Table = order.Table != null ? $"Table {order.Table.TableNumber}" : "Takeaway / Counter",
                Section = order.Table != null ? order.Table.Section : "Parcel",
                OrderType = order.Type == OrderType.DineIn ? "Dine-In" : "Takeaway",
